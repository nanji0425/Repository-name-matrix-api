...
鏈斂绛栧彲鑳介殢鏈嶅姟銆佹硶寰嬫硶瑙勬垨涓婃父瑕佹眰鍙樺寲鑰屾洿鏂般€傜户缁娇鐢?MatrixAPI 琛ㄧず浣犵悊瑙ｅ苟鎺ュ彈鏇存柊鍚庣殑鏀跨瓥銆?'),
  ...
  ('console_setting.announcements', '[{"id":1,"type":"success","content":"MatrixAPI OpenAI-compatible API routes are available.","publishDate":"2026-07-07T00:00:00+08:00","enabled":true},{"id":2,"type":"warning","content":"Only Alipay top-up is currently enabled.","publishDate":"2026-07-07T00:10:00+08:00","enabled":true},{"id":3,"type":"ongoing","content":"MatrixAPI documentation is available on /docs and will continue to be expanded.","publishDate":"2026-07-07T00:20:00+08:00","enabled":true}]'),
  ('MinTopUp', :'min_topup'),
...
update options
set value = replace(replace(value, 'default group ratio set to 1.4', 'default group ratio set to 1.0'), 'retail pricing is 40% above the upstream base configuration', 'pricing follows the upstream base configuration')
...
MatrixAPI 是面向开发者和企业团队的 OpenAI 兼容 AI 模型聚合网关。生产环境通过 kukuai 上游提供模型能力，并通过统一的 /v1 接口提供模型列表、聊天补全、图像生成等功能。
...
如需企业额度、代理结算、白标或模型渠道配置，请联系 3315419516@qq.com。请勿在邮件中发送完整 API Key、密码或其他敏感凭证。
...
我们可能因上游模型、网络、维护或安全原因调整模型可用性、价格和限流策略。重大变更会在站内公告中说明。

### 联系方式

服务问题请联系 3315419516@qq.com。提交工单或邮件时请勿发送完整 API Key 或密码。\$\$ where key = 'legal.user_agreement';
...
请自行保护 API Key。我们不会要求你通过邮件或聊天发送完整密钥。为提供网关服务，API 请求内容可能在必要的运行时范围内经过上游模型渠道处理；请勿提交不应被处理的敏感个人信息。

### 保存与删除

我们会在实现上述目的所需的期限内保存账户、调用和订单记录。你可以联系 3315419516@qq.com 申请账户相关信息的查询或删除；法律要求保留的记录除外。

### 联系方式

隐私问题请联系 3315419516@qq.com。我们会在核验账户归属后处理相关请求。\$\$ where key = 'legal.privacy_policy';
...
select
  ...
  1,
  'kukuai',
  'MatrixAPI upstream channel. Pricing follows the configured group ratio.',
  '{}',
  null
where not exists (
...
-- Ensure every model currently exposed by the canonical upstream channel also
