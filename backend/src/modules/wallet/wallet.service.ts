...
export class WalletService {
  ...
  async getRechargeConfig() {
    return {
      ...
      payName: 'ZPay 在线支付',
      payHint: '提交订单后将跳转至 ZPay 收银台，支付成功后余额自动到账。',
  ...
  async createRechargeOrder(userId: string, dto: RechargeDto) {
    ...
    const payType = dto.payType === 'ALIPAY' ? 'alipay' : 'wxpay';
    const paymentParams = {
      pid: this.getZpayPid(),
      type: payType,
  ...
  private getZpayReturnUrl() {
    return process.env.ZPAY_RETURN_URL || `${process.env.FRONTEND_URL || 'http://matrixapi.online'}/dashboard/balance`;
  }

  private getApiPublicUrl() {
    return (process.env.API_PUBLIC_URL || process.env.NEXT_PUBLIC_API_URL || 'http://matrixapi.online/api').replace(/\/$/, '');
  }
