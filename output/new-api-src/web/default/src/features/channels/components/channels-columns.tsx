...
function BalanceCell({ channel }: { channel: Channel }) {
  ...
  const canRefreshBalance =
    channel.type === 57 || supportsBalanceQuery(channel)
  ...
  const handleClickUpdate = async () => {
    if (isUpdating || !canRefreshBalance) {
      return
    }
  ...
  } else if (!canRefreshBalance) {
    remainingTooltipLabel = t('Balance query unavailable for this channel')
  }
          ...
          <TooltipTrigger
            render={
              <StatusBadge
                ...
                copyable={false}
                showDot={false}
                className={canRefreshBalance ? 'cursor-pointer' : 'cursor-default'}
                onClick={canRefreshBalance ? handleClickUpdate : undefined}
              />
            }
          />
          <TooltipContent>
            <p>{remainingTooltipLabel}</p>
            {channel.type !== 57 && canRefreshBalance && (
