...
function hasAdvancedSettingsValues(values: ChannelFormValues): boolean {
  return Boolean(
    ...
    Number(values.upstream_price_markup) > 0
  )
}
  ...
  const createModeFetcher = useCallback(async (): Promise<string[]> => {
    ...
    const response = await fetchModels({
      ...
      setting: JSON.stringify({
        ...
        force_format: form.getValues('force_format') === true,
        thinking_to_content: form.getValues('thinking_to_content') === true,
      }),
      header_override: form.getValues('header_override') || '',
      settings: form.getValues('settings') || '{}',
      model_mapping: form.getValues('model_mapping') || '',
    })
                        ...
                        {MODEL_FETCHABLE_TYPES.has(currentType) && (
                          ...
                          >
                            ...
                            >
                              <FormField
                                control={form.control}
                                name='upstream_price_markup'
                                render={({ field }) => (
                                  <FormItem>
                                    <FormLabel>
                                      {t('Upstream price markup')}
                                    </FormLabel>
                                    <FormControl>
                                    ...
                                    <FormDescription>
                                      {t(
                                        'Applied once to model price or model ratio when importing pricing from this upstream. Relative completion and cache ratios are unchanged.'
                                      )}
                                    </FormDescription>
                                    <FormMessage />
                                  </FormItem>
