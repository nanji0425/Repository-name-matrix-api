...
import { Loader2, Search, Info, ChevronDown, ListChecks } from 'lucide-react'
...
export function FetchModelsDialog({
  ...
  const allFetchedModelsSelected =
    fetchedModels.length > 0 &&
    fetchedModels.every((model) => selectedModels.includes(model))

  const toggleAllFetchedModels = () => {
    setSelectedModels((prev) => {
      if (allFetchedModelsSelected) {
        const fetchedSet = new Set(fetchedModels)
        return prev.filter((model) => !fetchedSet.has(model))
      }
      return normalizeModelNameList([...prev, ...fetchedModels])
    })
  }
