...
export const channelFormSchema = z
  .object({
    ...
    upstream_price_markup: z.number().min(0).max(1000).optional(),
  })
...
export function transformChannelToFormDefaults(
  ...
  if (channel.settings) {
    try {
      ...
      upstreamModelUpdateIgnoredModels = Array.isArray(
        ...
        : ''
      upstreamPriceMarkup =
        typeof parsed.upstream_price_markup === 'number'
          ? parsed.upstream_price_markup
          : 0
  ...
  return {
    ...
    upstream_price_markup: upstreamPriceMarkup,
...
function buildSettingsJSON(formData: ChannelFormValues): string {
  ...
  settingsObj.upstream_price_markup = Math.min(
    1000,
    Math.max(0, Number(formData.upstream_price_markup) || 0)
  )
