...
export async function handleTestChannel(
  ...
  try {
    ...
    const normalized: ChannelTestNormalizedResult =
      normalizeChannelTestResponse(
        response,
        i18next.t(ERROR_MESSAGES.TEST_FAILED)
      )
    const responseTime =
      normalized.responseTime ?? getChannelTestResponseTime(response)
    ...
    if (normalized.success) {
      ...
      onTestComplete?.(false, responseTime, errorMsg, normalized.errorCode)
    }
  } catch (_error: unknown) {
    const normalized = normalizeChannelTestResponse(
      _error,
      i18next.t(ERROR_MESSAGES.TEST_FAILED)
    )
    const errorMsg = normalized.error || i18next.t(ERROR_MESSAGES.TEST_FAILED)
    const target = getChannelTestLabel(options)
    if (!options?.silent) {
      toast.error(i18next.t('{{target}} test failed', { target }), {
        description: errorMsg,
