...
/**
 * Whether the provider has a supported balance endpoint. Multi-key channels
 * are intentionally excluded because one aggregate balance cannot be
 * attributed to a single credential.
 */
export function supportsBalanceQuery(channel: Pick<Channel, 'type' | 'channel_info'>): boolean {
  if (channel.channel_info?.is_multi_key) return false
  return new Set([1, 8, 10, 12, 13, 20, 25, 40, 43]).has(channel.type)
}
