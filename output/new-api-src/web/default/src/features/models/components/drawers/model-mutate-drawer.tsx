...
import { getLobeIcon, isImageIconSource } from '@/lib/lobe-icon'
  ...
  const handleIconUpload = useCallback(
    async (event: React.ChangeEvent<HTMLInputElement>) => {
      ...
      if (!['image/png', 'image/jpeg', 'image/webp'].includes(file.type)) {
        toast.error(t('Only PNG, JPG, or WEBP images are supported'))
        return
      }
      if (file.size > 2 * 1024 * 1024) {
        toast.error(t('Model icon must be 2 MiB or smaller'))
        return
      }

      setIsUploadingIcon(true)
      try {
        const response = await uploadModelIcon(file)
        const url = response.data?.url
        if (!response.success || !url) {
          toast.error(response.message || t('Upload failed'))
          return
        }
        form.setValue('icon', url, { shouldDirty: true, shouldValidate: true })
        toast.success(t('Model icon uploaded'))
      } catch {
        // The shared API interceptor already reports the request error.
      } finally {
          ...
          >
            ...
            <SideDrawerSection>
              ...
              <FormField
                ...
                name='icon'
                render={({ field }) => {
                  let iconButtonContent: React.ReactNode = (
                  ...
                  } else if (isImageIconSource(iconValue)) {
                    iconButtonContent = getLobeIcon(iconValue, 26)
                  }
                    ...
                    <FormItem>
                      ...
                      <div className='relative'>
                        ...
                        <Button
                          ...
                          variant='ghost'
                          size='icon'
                          className='absolute top-1/2 right-1 size-8 -translate-y-1/2 rounded-full p-0'
                          disabled={isUploadingIcon}
                          aria-label={t('Upload model icon')}
                          title={t('Upload model icon')}
                      ...
                      <FormDescription className='text-xs'>
                        {t('@lobehub/icons key or uploaded image')}
                      </FormDescription>
                      <FormMessage />
                    </FormItem>
                  )
