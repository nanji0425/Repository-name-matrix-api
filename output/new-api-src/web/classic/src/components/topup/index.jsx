...
function openPaymentTab() {
  const target = `matrixapi-payment-${Date.now()}-${Math.random().toString(36).slice(2)}`;
  return { target, window: window.open('', target) };
}

function closePaymentTab(paymentTab) {
...
const TopUp = () => {
  ...
  const onlineTopUp = async () => {
    ...
    const paymentTab = payWay === 'stripe' ? null : openPaymentTab();
      ...
      if (res !== undefined) {
        ...
        if (message === 'success') {
          if (payWay === 'stripe') {
            ...
            form.action = url;
            form.method = 'POST';
            form.target = paymentTab?.window ? paymentTab.target : '_self';
            for (let key in params) {
              let input = ownerDocument.createElement('input');
              input.type = 'hidden';
              input.name = key;
