...
func validateAnnouncements(announcementsStr string) error {
	...
	for i, ann := range list {
		...
		if enabled, exists := ann["enabled"]; exists {
			if _, ok := enabled.(bool); !ok {
				return fmt.Errorf("第%d个公告的启用状态必须为布尔值", i+1)
			}
		}
		if len(content) > 500 {
...
func GetAnnouncements() []map[string]interface{} {
	return getAnnouncementsAt(time.Now())
}

func getAnnouncementsAt(now time.Time) []map[string]interface{} {
	...
	for _, item := range list {
		...
		if publishTime := getPublishTime(item); !publishTime.IsZero() && publishTime.After(now) {
			continue
		}
