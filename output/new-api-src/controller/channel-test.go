...
// prepareChannelTestPricing keeps connectivity tests useful for newly added
// models. A test request must reach the upstream even when the administrator
// has not imported pricing yet; normal relay requests never set IsChannelTest
// and therefore retain the regular billing guard.
func prepareChannelTestPricing(info *relaycommon.RelayInfo) {
...
func testChannel(ctx context.Context, channel *model.Channel, testUserID int, testModel string, endpointType string, isStream bool) testResult {
	...
	prepareChannelTestPricing(info)
...
func TestChannel(c *gin.Context) {
	...
	if result.localErr != nil {
		...
		if result.newAPIError != nil {
			...
			resp["upstream_status"] = result.newAPIError.StatusCode
		}
	...
	if result.newAPIError != nil {
		c.JSON(http.StatusOK, gin.H{
			...
			"upstream_status": result.newAPIError.StatusCode,
		})
		return
	}
	c.JSON(http.StatusOK, gin.H{
