...
// applyUpstreamPriceMarkup applies a channel's configured uplift to the two
// base pricing fields. Completion/cache/image/audio values are relative
// multipliers; changing them as well would compound the markup.
func applyUpstreamPriceMarkup(data map[string]any, percent float64) map[string]any {
	...
	for _, field := range []string{"model_ratio", "model_price"} {
		...
		adjusted := make(map[string]any, len(values))
		for modelName, value := range values {
			if number, ok := asFloat64(value); ok {
				adjusted[modelName] = roundRatioValue(number * multiplier)
			} else {
				adjusted[modelName] = value
			}
		}
...
func FetchUpstreamRatios(c *gin.Context) {
	...
	markupByName := make(map[string]float64, len(upstreams))
	for i := range upstreams {
		u := &upstreams[i]
		if u.ID > 0 {
			if dbChannel, err := model.GetChannelById(u.ID, true); err == nil {
				u.MarkupPercent = dbChannel.GetOtherSettings().UpstreamPriceMarkup
			}
		} else if u.ID < 0 {
			u.MarkupPercent = 0
		}
		if u.MarkupPercent < 0 || u.MarkupPercent > 1000 {
			u.MarkupPercent = 0
		}
		uniqueName := u.Name
		if u.ID != 0 {
			uniqueName = fmt.Sprintf("%s(%d)", u.Name, u.ID)
		}
		markupByName[uniqueName] = u.MarkupPercent
	}
	...
	for r := range ch {
		if r.Err != "" {
			...
			r.Data = applyUpstreamPriceMarkup(r.Data, markupByName[r.Name])
			testResults = append(testResults, dto.TestResult{
				Name:   r.Name,
				Status: "success",
			})
