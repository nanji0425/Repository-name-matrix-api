...
func fetchChannelUpstreamModelIDs(channel *model.Channel) ([]string, error) {
	...
	if channel.Type == constant.ChannelTypeOllama {
		...
		names := normalizeModelNames(lo.Map(models, func(item ollama.OllamaModel, _ int) string {
			return item.Name
		}))
		model.SortModelNames(names)
		return names, nil
	}
	...
	if err != nil {
		if strings.Contains(strings.ToLower(baseURL), "kukuai.fyi") {
			return fetchKukuaiPublishedModelCatalog(baseURL)
		}
		return nil, err
	}
	...
	if len(ids) == 0 && strings.Contains(strings.ToLower(baseURL), "kukuai.fyi") {
		return fetchKukuaiPublishedModelCatalog(baseURL)
	}

	ids = normalizeModelNames(ids)
	model.SortModelNames(ids)
	return ids, nil
}

// fetchKukuaiPublishedModelCatalog is a fallback for Kukuai's API proxy. The
// proxy intentionally returns an empty /v1/models list for some account keys,
// while the public catalog remains the authoritative model list.
func fetchKukuaiPublishedModelCatalog(baseURL string) ([]string, error) {
	parsed, err := url.Parse(strings.TrimSpace(baseURL))
	if err != nil || parsed.Scheme == "" || parsed.Host == "" {
		return nil, fmt.Errorf("invalid Kukuai base URL: %w", err)
	}
	catalogURL := fmt.Sprintf("%s://%s/api/dist/site/models", parsed.Scheme, parsed.Host)
	...
	if response.StatusCode != http.StatusOK {
		return nil, fmt.Errorf("published catalog returned status %d", response.StatusCode)
	}
	var payload struct {
	...
	if !payload.Success {
		return nil, fmt.Errorf("published catalog returned success=false")
	}
	...
	models = normalizeModelNames(models)
	if len(models) == 0 {
		return nil, fmt.Errorf("published catalog returned no models")
	}
	model.SortModelNames(models)
	return models, nil
}
