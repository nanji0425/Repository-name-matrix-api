...
func init() {
	...
	for i := 1; i <= constant.ChannelTypeDummy; i++ {
		...
		adaptor.Init(meta)
		modelNames := append([]string(nil), adaptor.GetModelList()...)
		model.SortModelNames(modelNames)
		channelId2Models[i] = modelNames
	}
	openAIModels = lo.UniqBy(openAIModels, func(m dto.OpenAIModels) string {
		return m.Id
	})
	sort.SliceStable(openAIModels, func(i, j int) bool {
		return model.CompareModelNames(openAIModels[i].Id, openAIModels[j].Id)
	})
}
...
func ListModels(c *gin.Context, modelType int) {
	...
	model.SortModelNames(userModelNames)
