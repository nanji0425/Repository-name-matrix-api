...
// fetchModelsRequest is the non-persistent channel representation used while
// creating a channel. It intentionally carries the same transport settings as
// a saved channel so discovery exercises the exact upstream adapter path.
...
func buildFetchModelsChannel(req fetchModelsRequest) (*model.Channel, error) {
	...
	if baseURL == "" {
		baseURL = strings.TrimRight(constant.ChannelBaseURLs[req.Type], "/")
	}
	if baseURL == "" {
		return nil, errors.New("base URL is required for this channel type")
	}
	...
	return &model.Channel{
		...
		Key:            req.Key,
		BaseURL:        &baseURL,
		Setting:        optionalString(req.Setting),
		HeaderOverride: optionalString(req.HeaderOverride),
		OtherSettings:  req.OtherSettings,
		ModelMapping:   optionalString(req.ModelMapping),
	}, nil
}
...
func FetchModels(c *gin.Context) {
	...
	if err != nil {
		c.JSON(http.StatusOK, gin.H{
			"success": false,
			"message": fmt.Sprintf("failed to fetch upstream models: %s", err.Error()),
		})
		return
	}
	c.JSON(http.StatusOK, gin.H{"success": true, "data": fetchedModels})
	return
